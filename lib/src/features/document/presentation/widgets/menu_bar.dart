...
class _FileMenuButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return PopupMenuButton<int>(
      ...
      itemBuilder: (context) => [
        PopupMenuItem<int>(
          child: const PopUpMenuTile(isActive: true, title: 'New Document'),
          onTap: () {
            newDocumentPressed?.call();
          },
        ),
        PopupMenuItem<int>(
          child: const PopUpMenuTile(isActive: true, title: 'Open'),
          onTap: () {
      ...
      child: const Padding(
        ...
        child: Text('File', style: _menuTextStyle),
      ),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return PopupMenuButton<int>(
      ...
      itemBuilder: (context) => [
        ...
        PopupMenuItem<int>(
          value: 2,
          child: const PopUpMenuTile(icon: Icons.cut, title: 'Cut'),
          onTap: () {
            cutPressed?.call();
          },
        ),
        PopupMenuItem<int>(
          value: 3,
          child: const PopUpMenuTile(icon: Icons.copy, title: 'Copy'),
          onTap: () {
            copyPressed?.call();
          },
        ),
        PopupMenuItem<int>(
          value: 3,
          child: const PopUpMenuTile(icon: Icons.paste, title: 'Paste'),
          onTap: () {
            pastePressed?.call();
          },
        ),
...
class PopUpMenuTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: <Widget>[Icon(icon), const SizedBox(width: 8), Text(title)],
    );
  }
}
